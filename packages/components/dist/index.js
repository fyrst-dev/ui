import { defineComponent as V, computed as k, createBlock as Q, openBlock as B, resolveDynamicComponent as ee, normalizeClass as _, withCtx as re, createElementBlock as R, createCommentVNode as j, unref as v, renderSlot as y, createElementVNode as P, ref as O, watch as te, onMounted as oe, onUnmounted as ne, useSlots as ae, provide as x, withModifiers as se, toDisplayString as le } from "vue";
function M(e) {
  return typeof e == "object" && e != null && !Array.isArray(e);
}
var ie = (e) => typeof e == "object" && e !== null;
function W(e) {
  return Object.fromEntries(Object.entries(e ?? {}).filter(([r, o]) => o !== void 0));
}
var de = (e) => e === "base";
function ce(e) {
  return e.slice().filter((r) => !de(r));
}
function D(e) {
...
function ge(e) {
  return ue(be(5381, e) >>> 0);
}
var F = /\s*!(important)?/i;
function me(e) {
  return typeof e == "string" ? F.test(e) : !1;
}
function pe(e) {
  return typeof e == "string" ? e.replace(F, "").trim() : e;
}
function Y(e) {
...
function q(e, r, o = !0) {
  ...
  return $(
    e,
    (l) => Array.isArray(l) ? ve(l, n.breakpoints.keys) : l,
    {
...
function _e(e) {
  const { utility: r, hash: o, conditions: t = xe } = e, n = (s) => [r.prefix, s].filter(Boolean).join("-"), a = (s, l) => {
    let c;
    if (o) {
  ...
  return E(({ base: s, ...l } = {}) => {
    const c = Object.assign(l, s), g = q(c, e), f = /* @__PURE__ */ new Set();
    return $(g, (b, i) => {
      if (b == null) return;
      const u = me(b), [p, ...h] = t.shift(i), C = ce(h), I = r.transform(p, pe(ye(b)));
...
function Ce(e) {
  function r(n) {
    const a = ke(...n);
    return a.length === 1 ? a : a.map((s) => q(s, e));
  }
  function o(...n) {
    return A(...r(n));
  }
...
var Se = /([A-Z])/g, we = /^ms-/, Te = E((e) => e.startsWith("--") ? e : e.replace(Se, "-$1").replace(we, "-ms-").toLowerCase()), Be = "cm,mm,Q,in,pc,pt,px,em,ex,ch,rem,lh,rlh,vw,vh,vmin,vmax,vb,vi,svw,svh,lvw,lvh,dvw,dvh,cqw,cqh,cqi,cqb,cqmin,cqmax,%";
`${Be.split(",").join("|")}`;
var Ie = (e = {}) => {
  const r = (n) => {
    var a;
    return {
      ...
      compoundVariants: e.compoundVariants ? Oe(e.compoundVariants, n) : []
    };
...
}, Oe = (e, r) => e.filter((o) => o.css[r]).map((o) => ({ ...o, css: o.css[r] }));
function H(e, ...r) {
...
const X = (e) => U.get(e) || e, Z = {
  ...
  utility: {
    transform: (e, r) => {
      const o = X(e);
      return { className: `${K.get(o) || Te(o)}_${Y(r)}` };
    },
    hasShorthand: !0,
    toHash: (e, r) => r(e.join(":")),
    resolveShorthand: X
...
function z(e) {
  const { base: r, variants: o, defaultVariants: t, compoundVariants: n } = N(e), a = (i) => ({ ...t, ...W(i) });
  function s(i = {}) {
    var C;
    const u = a(i);
    let p = { ...r };
    for (const [I, d] of Object.entries(u))
      (C = o[I]) != null && C[d] && (p = T(p, o[I][d]));
    const h = Le(n, u);
    return T(p, h);
  }
  function l(i) {
    const u = N(i.config), p = Re(i.variantKeys, Object.keys(o));
    return z({
      base: T(r, u.base),
      variants: Object.fromEntries(
        p.map((h) => [h, T(o[h], u.variants[h])])
      ),
...
function Le(e, r) {
  let o = {};
  return e.forEach((t) => {
    Object.entries(t).every(([a, s]) => a === "css" ? !0 : (Array.isArray(s) ? s : [s]).some((c) => r[a] === c)) && (o = T(o, t.css));
  }), o;
}
function J(e) {
  const r = Object.entries(Ie(e)).map(([b, i]) => [b, z(i)]), o = e.defaultVariants ?? {}, t = r.reduce((b, [i, u]) => (e.className && (b[i] = u.config.className), b), {});
  function n(b) {
    const i = r.map(([u, p]) => [u, De(p(b), t[u])]);
    return Object.fromEntries(i);
  }
  function a(b) {
    const i = r.map(([u, p]) => [u, p.raw(b)]);
    return Object.fromEntries(i);
  }
...
const Xe = ["innerHTML"], Ze = /* @__PURE__ */ V({
  __name: "Button",
  props: {
    label: { default: null },
    icon: { default: null },
  ...
  setup(e) {
    const r = e, o = k(() => r.to ? "a" : "button"), t = k(() => o.value === "button" && r.type ? r.type : o.value === "button" ? "button" : null), n = k(() => r.external && o.value === "a" ? "_blank" : null), a = k(() => r.external && o.value === "a" ? "noopener noreferrer" : null), s = z({
      base: {
        display: "inline flex",
        alignItems: "center",
...
function Ne(e, r, o) {
  const t = O(null), n = O(0), a = O([]), s = O(null), l = O(null), c = {
    itemActiveClass: "active",
    itemVisibleClass: "visible"
  }, g = k(() => (t.value ?? []).length > 0 ? t.value[n.value] : null), f = k(() => {
    var d;
    return Math.ceil((((d = t.value) == null ? void 0 : d.length) ?? 0) / a.value.length);
  }), b = k(() => {
    var m;
    const d = ((m = t.value) == null ? void 0 : m.length) ?? 0;
    return d > 0 && a.value.length === d;
  });
  te(g, (d, m) => {
    d && d.classList.add(c.itemActiveClass), m && m.classList.remove(c.itemActiveClass);
  });
  const i = (d) => {
    if (!t.value) return null;
    if (d === "first") {
      const m = a.value[0];
      return m ? Array.from(t.value).indexOf(m) - 1 : null;
    }
    if (d === "last") {
    ...
    const d = s.value === "next" ? "end" : "start", m = s.value === "next" ? n.value + 1 : n.value - 1;
    if (!t.value || t.value[m] === void 0) {
      console.warn("No more items to scroll", s.value);
      return;
    }
    t.value[m].style.scrollSnapAlign = d, t.value[m].scrollIntoView({ behavior: "smooth" }), n.value = m;
  }, p = () => {
    ...
    s.value = "next", e.scrollMode === "item" ? u() : e.scrollMode === "viewport" ? p() : console.warn("No more items to scroll next");
  }, C = () => {
    s.value = "prev", e.scrollMode === "item" ? u() : e.scrollMode === "viewport" ? p() : console.warn("No more items to scroll prev");
  }, I = (d) => {
    d.deltaY > 0 ? h() : C();
  };
  return oe(() => {
    var d;
    t.value = o.default ? ((d = r.value) == null ? void 0 : d.querySelectorAll(":scope > *")) ?? null : null, l.value = new IntersectionObserver((m) => {
      m.forEach((S) => {
        S.isIntersecting ? (S.target.dataset.carouselVisibility = "visible", S.target.classList.add(c.itemVisibleClass)) : (delete S.target.dataset.carouselVisibility, S.target.classList.remove(c.itemVisibleClass));
      }), a.value = Array.from(t.value ?? []).filter((S) => S.dataset.carouselVisibility === "visible");
    }, {
      root: r.value,
      rootMargin: "0px",
      threshold: 1
    }), t.value && l.value && t.value.forEach((m) => l.value.observe(m));
  }), ne(() => {
...
const Qe = /* @__PURE__ */ V({
  ...
  setup(e) {
    ...
    return x("carousel", t), x("handleNext", n), x("handlePrev", a), x("carouselItems", l), x("carouselItem", c), x("visibleCarouselItems", g), x("currentCarouselItemIndex", f), x("scrollDirection", b), x("allItemsVisible", i), (u, p) => (B(), R("div", {
      class: _(v(w)(
        v(w).raw({
          position: "relative",
          display: "flex",
...
}), Fe = ["for"], Ye = ["id", "name", "placeholder", "value", "disabled", "required", "autocomplete"], $e = /* @__PURE__ */ V({
  __name: "FieldText",
  props: {
    label: { default: null },
...
function Ge(e, r) {
  return " This should be the field base ";
}
const Ke = /* @__PURE__ */ qe(He, [["render", Ge]]), er = {
  Text: $e,
  Base: Ke
};
export {
