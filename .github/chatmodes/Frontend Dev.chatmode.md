...
**Card/** - Card container system (namespace: `Card.*`):
- **CardRoot.vue** - Main container with border variants and slot composition
- **CardBody.vue** - Card content wrapper

**Carousel/** - Horizontal scrolling system (namespace: `Carousel.*`):
- **CarouselRoot.vue** - Main scrollable container with composable logic
- **CarouselItem.vue** - Individual carousel slide
- **CarouselNavigation.vue** - Next/prev controls

**Flyout/** - Overlay system (namespace: `Flyout.*`):
- **FlyoutRoot.vue** - Hover-triggered overlay container
- **FlyoutDropdown.vue** - Positioned dropdown variant

**Hero/** - Hero section system (namespace: `Hero.*`):
- **HeroLead.vue** - Hero section with headline, description, and actions
...
- Usage (flat): `<FyrstButton>`, `<FyrstCardRoot>`, `<FyrstCardBody>`, `<FyrstFieldInput>`, etc.
- Direct imports support namespace: `import Card from '@fyrst/ui-components'` → `<Card.Root>`, `<Card.Body>`
...
### Naming
- **Components**: PascalCase (Button.vue, CardRoot.vue, FieldInput.vue)
- **Component families**: Main components use `Root` suffix (CardRoot.vue, CarouselRoot.vue, FlyoutRoot.vue)
